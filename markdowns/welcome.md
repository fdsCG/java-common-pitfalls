...
The operator `==` compares object references and returns `true` if the two operands point to the same object.  
The `equals` method returns `true` if both objects are equivalent.
